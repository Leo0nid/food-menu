...
var _nostics = await jitiImport("nostics");
var _nodeProcess = _interopRequireDefault(await jitiImport("node:process"));
var _ansi = await jitiImport("nostics/formatters/ansi");
var _utils = await jitiImport("consola/utils");
var _verkit = await jitiImport("verkit");
...
var _utils2 = await jitiImport("pathe/utils");
...
var _promises = await jitiImport("node:fs/promises");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
const reporters = [/* @__PURE__ */(0, _nostics.createConsoleReporter)(_nodeProcess.default.env.NODE_ENV === "test" ? void 0 : { formatter: (0, _ansi.ansiFormatter)(_utils.colors) })];
//#endregion
...
* Kit API diagnostics.
*/
const kitDiagnostics = /* #__PURE__ */(0, _nostics.defineDiagnostics)({
  docsBase,
  reporters,
  codes: {
    NUXT_B8001: {
...
const asyncNuxtStorage = (0, _unctx.getContext)("asyncNuxtStorage", {
  asyncContext: true,
...
function checkNuxtVersion(version, nuxt = useNuxt()) {
  ...
  return (0, _verkit.satisfies)(normalizeSemanticVersion(nuxtVersion), version, { includePrerelease: true });
}
/**
...
async function checkNuxtCompatibility(constraints, nuxt = useNuxt()) {
  ...
  if (constraints.builder && typeof nuxt.options.builder === "string") {
    ...
    if (currentBuilder in constraints.builder) {
      ...
      nuxt.options.workspaceDir, "file:///C:/Users/Leonid/Desktop/Projects/food-menu/node_modules/@nuxt/icon/node_modules/@nuxt/kit/dist/index.mjs"])

      {
        const builderVersion = await (0, _pkgTypes.readPackageJSON)(nuxt.options.builder, { parent }).then((r) => r.version).catch(() => void 0);
        if (builderVersion) {
          if (!(0, _verkit.satisfies)(normalizeSemanticVersion(builderVersion), constraint, { includePrerelease: true })) issues.push({
            name: "builder",
...
function _defineNuxtModule(definition) {
  ...
  async function getOptions(inlineOptions, nuxt = useNuxt()) {
    ...
    if (module.schema) {
      const { applyDefaults } = await Promise.resolve().then(() => jitiImport("untyped").then((m) => _interopRequireWildcard(m)));
  ...
  async function normalizedModule(inlineOptions, nuxt = tryUseNuxt()) {
    ...
    if (nuxt.options.debug && nuxt.options.debug.modules) logger.info(`Module \`${moduleName}\` took \`${setupTime}ms\` to setup.`);
    if (res === false) return false;
    return (0, _defu.defu)(res, { timings: { setup: setupTime } });
  }
...
const distURL = "file:///C:/Users/Leonid/Desktop/Projects/food-menu/node_modules/@nuxt/icon/node_modules/@nuxt/kit/dist/index.mjs".replace(/\/dist\/.*$/, "/");
function getUserCaller() {
  if (!import.meta.dev) return null;
  const { source, line, column } = (0, _errx.captureStackTrace)().find((entry) => !entry.source.startsWith(distURL)) ?? {};
...
function getLayerDirectories(nuxt = useNuxt()) {
  return nuxt.options._layers.map((layer) => {
    ...
    const directories = {
      root,
      shared: withTrailingSlash$2((0, _pathe.resolve)(root, resolveAlias(config.dir?.shared || "shared", nuxt.options.alias))),
      server: withTrailingSlash$2((0, _pathe.resolve)(src, resolveAlias(config.serverDir || "server", nuxt.options.alias))),
      modules: withTrailingSlash$2((0, _pathe.resolve)(src, resolveAlias(config.dir?.modules || "modules", nuxt.options.alias))),
      public: withTrailingSlash$2((0, _pathe.resolve)(src, resolveAlias(config.dir?.public || "public", nuxt.options.alias))),
      app: src,
      appLayouts: withTrailingSlash$2((0, _pathe.resolve)(src, resolveAlias(config.dir?.layouts || "layouts", nuxt.options.alias))),
      appMiddleware: withTrailingSlash$2((0, _pathe.resolve)(src, resolveAlias(config.dir?.middleware || "middleware", nuxt.options.alias))),
      appPages: withTrailingSlash$2((0, _pathe.resolve)(src, resolveAlias(config.dir?.pages || "pages", nuxt.options.alias))),
      appPlugins: withTrailingSlash$2((0, _pathe.resolve)(src, resolveAlias(config.dir?.plugins || "plugins", nuxt.options.alias)))
    };
...
function isIgnored(pathname, _stats, nuxt = tryUseNuxt()) {
  ...
  const relativePath = (0, _pathe.relative)(nuxt.options.rootDir, pathname);
...
const distDirURL = new URL(".", "file:///C:/Users/Leonid/Desktop/Projects/food-menu/node_modules/@nuxt/icon/node_modules/@nuxt/kit/dist/index.mjs");
//#endregion
...
* @internal
*/
const DEFAULT_JSX_FILE_EXTENSIONS = exports.DEFAULT_JSX_FILE_EXTENSIONS = [".tsx", ".jsx"];
/** Default extensions for JavaScript/TypeScript files, in order of resolution priority. */
const DEFAULT_JS_FILE_EXTENSIONS = exports.DEFAULT_JS_FILE_EXTENSIONS = [
".mjs",
".js",
".cjs",
".mts",
...
function resolveAlias(path, alias) {
  ...
  return (0, _utils2.resolveAlias)(path, alias);
}
/**
...
function createResolver(base) {
  ...
  base = base.toString();
  if (base.startsWith("file://")) base = (0, _pathe.dirname)((0, _nodeUrl.fileURLToPath)(base));
  return {
...
async function _resolvePathType(path, opts = {}, skipFs = false) {
  ...
  if (skipFs) return;
  const stats = await _nodeFs.promises.stat(path).catch(() => null);
  if (stats) return {
    path,
...
async function _resolvePathGranularly(path, opts = { type: "file" }) {
  ...
  const nuxt = tryUseNuxt();
  const cwd = opts.cwd || (nuxt ? nuxt.options.rootDir : _nodeProcess.default.cwd());
  const extensions = opts.extensions || (nuxt ? nuxt.options.extensions : [...DEFAULT_JS_FILE_EXTENSIONS, ".json"]);
  const modulesDir = nuxt ? nuxt.options.modulesDir : [];
  path = (0, _utils2.resolveAlias)(path, opts.alias ?? nuxt?.options.alias ?? {});
  if (!(0, _pathe.isAbsolute)(path)) path = (0, _pathe.resolve)(cwd, path);
  const res = await _resolvePathType(path, opts);
  if (res && res.type === opts.type) return res;
  if (opts.type === "file") {
    for (const ext of extensions) {
      ...
      const indexPath = await _resolvePathType((0, _pathe.join)(path, "index" + normalizedExt), opts, res?.type !== "dir");
...
async function existsSensitive(path) {
  return new Set(await _nodeFs.promises.readdir((0, _pathe.dirname)(path)).catch(() => [])).has((0, _pathe.basename)(path));
}
...
async function resolveFiles(path, pattern, opts = {}) {
  const files = [];
  for (const p of await (0, _tinyglobby.glob)(pattern, {
    cwd: path,
...
function tryResolveModule(id, url = "file:///C:/Users/Leonid/Desktop/Projects/food-menu/node_modules/@nuxt/icon/node_modules/@nuxt/kit/dist/index.mjs") {
...
function resolveModule(id, options) {
  return (0, _exsolve.resolveModulePath)(id, {
    from: options?.url ?? options?.paths ?? ["file:///C:/Users/Leonid/Desktop/Projects/food-menu/node_modules/@nuxt/icon/node_modules/@nuxt/kit/dist/index.mjs"],
...
function requireModule(id, opts) {
  ...
  const resolvedPath = resolveModule(id, opts);
  return (0, _jiti.createJiti)("file:///C:/Users/Leonid/Desktop/Projects/food-menu/node_modules/@nuxt/icon/node_modules/@nuxt/kit/dist/index.mjs", { interopDefault: opts?.interopDefault !== false })((0, _nodeUrl.pathToFileURL)(resolvedPath).href);
}
/**
...
async function installModules(modulesToInstall, resolvedModulePaths, nuxt = useNuxt()) {
  ...
  for (const l of nuxt.options._layers) {
    ...
    if (!NODE_MODULES_RE.test(srcDir)) localLayerModuleDirs.push((0, _pathe.resolve)(srcDir, l.config?.dir?.modules || "modules").replace(/\/?$/, "/"));
  }
  ...
  for (const [key, options] of modulesToInstall) {
    ...
    for (const [name, value] of Object.entries(dependencyMeta)) {
      ...
      if (value.version) {
        ...
        if (pkg?.version && !(0, _verkit.satisfies)(pkg.version, value.version, { includePrerelease: true })) {
  ...
  for (const { nuxtModule, meta = {}, moduleToInstall, buildTimeModuleMeta, resolvedModulePath, inlineOptions } of resolvedModules) {
    ...
    if (optionsFns.size > 0) {
      ...
      if (configKey) nuxt.options[configKey] = (0, _defu.defu)(...overrides, nuxt.options[configKey], ...defaults);
    }
    ...
    await callModule(nuxt, nuxtModule, inlineOptions, {
      meta: (0, _defu.defu)({ disabled: isDisabled }, meta, buildTimeModuleMeta),
      nameOrPath: path,
...
async function installModule(moduleToInstall, inlineOptions, nuxt = useNuxt()) {
  ...
  if (configKey && nuxt._moduleOptionsFunctions) {
    ...
    if (optionsFns.length > 0) {
      ...
      mergedOptions = (0, _defu.defu)(inlineOptions, ...overrides, nuxt.options[configKey], ...defaults);
...
function resolveModuleWithOptions(definition, nuxt) {
  ...
  return {
    module,
    resolvedPath: (0, _exsolve.resolveModulePath)(modAlias, {
      try: true,
...
function getSharedJiti(nuxt) {
  ...
  if (!jiti) {
    jiti = Promise.resolve().then(() => jitiImport("jiti").then((m) => _interopRequireWildcard(m))).then(({ createJiti }) => createJiti(nuxt.options.rootDir, { alias: nuxt.options.alias }));
...
async function loadNuxtModuleInstance(nuxtModule, nuxt = useNuxt()) {
  ...
  if ((0, _ufo.isRelative)(nuxtModule)) nuxtModule = (0, _pathe.resolve)(nuxt.options.rootDir, nuxtModule);
  let src;
  try {
    src = (0, _exsolve.resolveModuleURL)(nuxtModule, {
      from: nuxt.options.modulesDir.map((m) => directoryToURL(m.replace(/\/node_modules\/?$/, "/"))),
      suffixes: [
      "nuxt",
      "nuxt/index",
  ...
  if ((0, _nodeFs.existsSync)(moduleMetadataPath)) buildTimeModuleMeta = JSON.parse(await _nodeFs.promises.readFile(moduleMetadataPath, "utf-8"));
  return {
...
async function callLifecycleHooks(nuxtModule, meta = {}, inlineOptions, nuxt = useNuxt()) {
  ...
  const previousVersion = (0, _rc.read)({
    dir: nuxt.options.rootDir,
    name: ".nuxtrc"
  })?.setups?.[meta.name];
  try {
    if (!previousVersion) await nuxtModule.onInstall?.(nuxt);else
    if ((0, _verkit.isGreater)(meta.version, previousVersion)) await nuxtModule.onUpgrade?.(nuxt, inlineOptions, previousVersion);
    if (previousVersion !== meta.version) (0, _rc.update)({ setups: { [meta.name]: meta?.version } }, {
      dir: nuxt.options.rootDir,
      name: ".nuxtrc"
    });
  } catch (e) {
...
async function callModule(nuxt, nuxtModule, moduleOptions = {}, options) {
  ...
  if (typeof modulePath === "string") {
    ...
    if (parsed.name) {
      const subpath = (await (0, _mlly.lookupNodeModuleSubpath)(modulePath)) || ".";
      entryPath = (0, _pathe.join)(parsed.name, subpath === "./" ? "." : subpath);
    }
    if (res !== false) {
      ...
      if (moduleRoot !== nameOrPath && !localLayerModuleDirs.some((dir) => directory.startsWith(dir))) nuxt.options.modulesDir.push((0, _pathe.join)(moduleRoot, "node_modules"));
    }
  }
  if (nameOrPath) {
...
async function hasNuxtModuleCompatibility(module, semverVersion, nuxt = useNuxt()) {
  ...
  return (0, _verkit.satisfies)(normalizeSemanticVersion(version), semverVersion, { includePrerelease: true });
}
/**
...
async function loadNuxtConfig(opts) {
  const rootCwd = (0, _pathe.resolve)(opts.cwd || _nodeProcess.default.cwd());
  const localLayers = (await (0, _tinyglobby.glob)("layers/*", {
    onlyDirectories: true,
    cwd: rootCwd
  })).map((d) => (0, _ufo.withTrailingSlash)(d)).sort((a, b) => b.localeCompare(a));
  opts.overrides = (0, _defu.defu)(opts.overrides, { _extends: localLayers });
  const autoScanSources = new Set(localLayers);
  const localLayerDirs = new Set(localLayers.map((dir) => canonicalLayerDir((0, _pathe.resolve)(rootCwd, (0, _ufo.withoutTrailingSlash)(dir)))));
  const extendsLocalLayerOrder = [];
  if (opts.dotenv !== false) await (0, _c.setupDotenv)({
    cwd: opts.cwd || _nodeProcess.default.cwd(),
    ...(typeof opts.dotenv === "object" ? opts.dotenv : {})
  });
  const schemaPromise = loadNuxtSchema(opts.cwd || _nodeProcess.default.cwd());
  const seenLayerDirs = /* @__PURE__ */new Set();
  const resolved = await withDefineNuxtConfig(() => (0, _c.loadConfig)({
    name: "nuxt",
    ...
    async resolve(source, resolveOptions) {
      ...
      const base = resolveOptions.cwd ? (0, _pathe.resolve)(resolveOptions.cwd) : rootCwd;
      const aliased = resolveLayerExtendsAlias(source, rootCwd);
      const path = aliased ?? (0, _pathe.resolve)(base, source);
      if (!(0, _nodeFs.existsSync)(path)) return;
      ...
      if (aliased) {
        const layer = await (0, _c.loadConfig)({
          cwd: aliased,
          name: "nuxt",
          configFile: "nuxt.config",
          rcFile: false,
  ...
  const nuxtConfig = (0, _klona.klona)(resolved.config);
  ...
  if (!opts.overrides?._prepare && !nuxtConfig.dev && !nuxtConfig.buildDir && (0, _nodeFs.existsSync)(defaultBuildDir)) nuxtConfig.buildDir = (0, _pathe.join)(nuxtConfig.rootDir, "node_modules/.cache/nuxt/.nuxt");
  ...
  const localRelativePaths = new Set(localLayers.map((layer) => (0, _ufo.withoutTrailingSlash)(layer)));
...
function canonicalLayerDir(path) {
  return (0, _pathe.normalize)((0, _nodeFs.realpathSync)((0, _nodeFs.statSync)(path).isDirectory() ? path : (0, _pathe.dirname)(path)));
}
...
function resolveLayerExtendsAlias(source, rootDir) {
  for (const alias of LAYER_EXTENDS_ALIASES) {
    ...
    if (source.startsWith(`${alias}/`)) return (0, _pathe.join)(rootDir, source.slice(alias.length + 1));
  }
}
/**
...
function reorderLocalLayersByExtends(layers, extendsOrder, localLayerDirs) {
  const layerDir = (layer) => {
    const dir = (0, _ufo.withoutTrailingSlash)(layer.config?.rootDir ?? layer.cwd ?? "");
    try {
      return (0, _pathe.normalize)((0, _nodeFs.realpathSync)(dir));
    } catch {
      return (0, _pathe.normalize)(dir);
    }
  };
...
function loadNuxtSchema(cwd) {
  ...
  const urls = [url];
  const nuxtPath = (0, _exsolve.resolveModuleURL)("nuxt", {
    try: true,
    from: url
  }) ?? (0, _exsolve.resolveModuleURL)("nuxt-nightly", {
    try: true,
    from: url
  });
  if (nuxtPath) urls.unshift(nuxtPath);
  return ((specifier) => new Promise((r) => r(`${specifier}`)).then((s) => jitiImport(s).then((m) => _interopRequireWildcard(m))))((0, _exsolve.resolveModuleURL)("@nuxt/schema", {
    try: true,
    from: urls
...
async function loadNuxt(opts) {
  ...
  const resolvedPath = ["nuxt-nightly", "nuxt"].reduce((resolvedPath, pkg) => {
    const path = (0, _exsolve.resolveModulePath)(pkg, {
      try: true,
  ...
  const { loadNuxt } = await ((specifier) => new Promise((r) => r(`${specifier}`)).then((s) => jitiImport(s).then((m) => _interopRequireWildcard(m))))((0, _nodeUrl.pathToFileURL)(resolvedPath).href).then((r) => (0, _mlly.interopDefault)(r));
...
function setGlobalHead(head) {
  const nuxt = useNuxt();
  nuxt.options.app.head = (0, _defu.defu)(head, nuxt.options.app.head);
}
//#endregion
...
function updateAppConfig(appConfig) {
  const nuxt = useNuxt();
  Object.assign(nuxt.options.appConfig, (0, _defu.defu)(appConfig, nuxt.options.appConfig));
}
//#endregion
//#region src/nitro.ts
...
function updateRuntimeConfig(runtimeConfig) {
  const nuxt = useNuxt();
  Object.assign(nuxt.options.nitro.runtimeConfig, (0, _defu.defu)(runtimeConfig, nuxt.options.nitro.runtimeConfig));
  try {
...
function getEnv(key, opts, env = _nodeProcess.default.env) {
  ...
  return (0, _destr.default)(env[opts.prefix + envKey] ?? env[opts.altPrefix + envKey]);
}
...
function applyEnv(obj, opts, parentKey = "") {
  for (const key in obj) {
    ...
    if (_isObject(obj[key])) {if (_isObject(envValue)) {
      ...
      } else if (envValue === void 0) applyEnv(obj[key], opts, subKey);else
      obj[key] = envValue ?? obj[key];} else
...
function groupByEnforce(plugins, defaultEnforce) {
  ...
  for (const plugin of plugins) {
    ...
    if (group) group.push(plugin);else
...
* @internal
*/
const componentDiagnostics = exports.componentDiagnostics = /* #__PURE__ */(0, _nostics.defineDiagnostics)({
  docsBase,
  reporters,
  codes: {
    NUXT_B3001: {
...
function addComponentExports(opts) {
  ...
  nuxt.hook("components:dirs", async () => {
    ...
    for (const name of names) components.push(normalizeComponent({
      name: (0, _scule.pascalCase)([opts.prefix || "", name === "default" ? "" : name]),
      export: name,
      ...opts
    }));
  });
...
* @internal
*/
const pageDiagnostics = exports.pageDiagnostics = /* #__PURE__ */(0, _nostics.defineDiagnostics)({
  docsBase,
  reporters,
  codes: {
    NUXT_B4001: {
...
function isFile(path) {
  return _nodeFs.promises.stat(path).then((s) => s.isFile(), () => false);
}
/**
...
function resolveRoot(basePkg, from) {
  ...
  const promise = (async () => {
    try {
      const r = (0, _exsolve.resolveModulePath)(basePkg, {
        from,
        ...TYPE_RESOLVE_OPTIONS
      });
      return (0, _pathe.dirname)(await (0, _pkgTypes.resolvePackageJSON)(r));
    } catch {
      return;
    }
  })();
...
async function resolveTypePaths(packages, searchPaths) {
  ...
  return (await Promise.allSettled(packages.map(async (pkg) => {
    ...
    const resolved = (0, _exsolve.resolveModulePath)(pkg, {
      from,
      try: true,
      ...TYPE_RESOLVE_OPTIONS
    });
...
function addTemplate(_template) {
  ...
  try {
    ...
    if (source) {
      ...
      if ((0, _nodeFs.existsSync)(path)) template._path = path;
    }
  } catch {}
...
function addTypeTemplate(_template, context) {
  ...
  if (!context || context.nuxt || context.shared) nuxt.options.vite.vue = (0, _defu.defu)(nuxt.options.vite.vue, { script: { globalTypeFiles: [template.dst] } });
...
function normalizeTemplate(template, buildDir) {
  ...
  if (typeof template === "string") template = { src: template };else
  template = { ...template };
  if (template.src) {
    if (!(0, _nodeFs.existsSync)(template.src)) throw kitDiagnostics.NUXT_B8009({ template: template.src });
...
function resolveLayerPaths(dirs, projectBuildDir) {
  ...
  return {
    ...
    (0, _pathe.join)(relativeRootDir, `test/nuxt/**/*`),
    (0, _pathe.join)(relativeRootDir, `tests/nuxt/**/*`),
    ...
    globalDeclarations: [(0, _pathe.join)(relativeRootDir, `*.d.ts`), (0, _pathe.join)(relativeRootDir, `layers/*/*.d.ts`)]
  };
}
...
async function _generateTypes(nuxt) {
  ...
  for (const path of modulePaths) {
    ...
    if (!path.includes("node_modules") && path.startsWith(rootDirWithSlash)) {
      include.add((0, _pathe.join)(relative, "runtime"));
      include.add((0, _pathe.join)(relative, "dist/runtime"));
      nodeInclude.add((0, _pathe.join)(relative, "*.*"));
    }
    legacyInclude.add((0, _pathe.join)(relative, "runtime"));
    legacyInclude.add((0, _pathe.join)(relative, "dist/runtime"));
    nodeExclude.add((0, _pathe.join)(relative, "runtime"));
    nodeExclude.add((0, _pathe.join)(relative, "dist/runtime"));
    exclude.add((0, _pathe.join)(relative, "runtime/server"));
    exclude.add((0, _pathe.join)(relative, "dist/runtime/server"));
    exclude.add((0, _pathe.join)(relative, "*.*"));
    exclude.add((0, _pathe.join)(relative, "dist/*.*"));
    legacyExclude.add((0, _pathe.join)(relative, "runtime/server"));
    legacyExclude.add((0, _pathe.join)(relative, "dist/runtime/server"));
  }
  ...
  for (const parent of nestedModulesDirs) hasTypescriptVersionWithModulePreserve ??= await (0, _pkgTypes.readPackageJSON)("typescript", { parent }).then((r) => r?.version && (0, _verkit.isGreaterOrEqual)(r.version, "5.4.0")).catch(() => void 0);
  ...
  for (const alias in aliases) {
    ...
    let stats = await _nodeFs.promises.stat(absolutePath).catch(() => null);
    if (!stats) {
  ...
  async function resolveConfig(tsConfig) {
    for (const alias in tsConfig.compilerOptions.paths) {
      ...
      tsConfig.compilerOptions.paths[alias] = [...new Set(await Promise.all(paths.map(async (path) => {
        if (!(0, _pathe.isAbsolute)(path)) return path;
        return (await _nodeFs.promises.stat(path).catch(() => null))?.isFile() ? getPathSubstitution(path, nuxt.options.buildDir) : relativeWithDot(nuxt.options.buildDir, path);
      })))];
    }
    ...
    tsConfig.include = [...new Set(tsConfig.include.map((p) => (0, _pathe.isAbsolute)(p) ? relativeWithDot(nuxt.options.buildDir, p) : p))];
    tsConfig.exclude = [...new Set(tsConfig.exclude.map((p) => (0, _pathe.isAbsolute)(p) ? relativeWithDot(nuxt.options.buildDir, p) : p))];
  }
  await Promise.all([
...
async function writeIfChanged(path, contents) {
  if ((await _nodeFs.promises.readFile(path, "utf8").catch(() => void 0)) === contents) return;
  await _nodeFs.promises.writeFile(path, contents);
}
/**
...
function sortTsPaths(paths, layerDirs, buildDir, hoist) {
  ...
  for (const pathKey in paths) {
    ...
    if (layerDirs.length && paths[pathKey].some((target) => isPathUnderLayerDirs(target, buildDir, layerDirs))) {if (pathKey.startsWith("#layers")) genericLayerPaths[pathKey] = paths[pathKey];else
      customLayerPaths[pathKey] = paths[pathKey];} else
...
function isPathUnderLayerDirs(target, buildDir, layerDirs) {
  const absolute = withTrailingSlash$1((0, _pathe.resolve)(buildDir, target.replace(PATH_WILDCARD_RE, "")));
  return layerDirs.some((dir) => absolute.startsWith(dir));
}
function renderReference(ref, baseDir) {
  return `/// <reference ${"path" in ref ? `path="${(0, _pathe.isAbsolute)(ref.path) ? (0, _pathe.relative)(baseDir, ref.path) : ref.path}"` : `types="${ref.types}"`} />`;
}
...
function addLayout(template, name) {
  ...
  nuxt.hook("app:templates", (app) => {
    if (layoutName in app.layouts) {
      const relativePath = (0, _utils2.reverseResolveAlias)(app.layouts[layoutName].file, {
        ...(nuxt?.options.alias || {}),
        ...strippedAtAliases
  ...
  nuxt.hook("app:resolve", (app) => {
    for (const middleware of middlewares) {
      ...
      if (find >= 0) {
        ...
        if (options.override === true) app.middleware[find] = { ...middleware };else
        pageDiagnostics.NUXT_B4013({
          name: middleware.name,
          foundPath
        });
      } else if (options.prepend === true) app.middleware.unshift({ ...middleware });else
...
* @internal
*/
const pluginDiagnostics = exports.pluginDiagnostics = /* #__PURE__ */(0, _nostics.defineDiagnostics)({
  docsBase,
  reporters,
  codes: {
    NUXT_B2001: {
...
function normalizePlugin(plugin) {
  if (typeof plugin === "string") plugin = { src: plugin };else
  plugin = { ...plugin };
  ...
  if (!(0, _nodeFs.existsSync)(plugin.src) && (0, _nodePath.isAbsolute)(plugin.src)) try {
    plugin.src = (0, _exsolve.resolveModulePath)(plugin.src, { extensions: tryUseNuxt()?.options.extensions ?? DEFAULT_JS_FILE_EXTENSIONS });
  } catch {}
...
function recoverThrottledChanges(watcher) {
  ...
  */
  ...
  const toKey = (path) => cwd ? (0, _nodePath.resolve)(cwd, path) : path;
  /**
  ...
  */
  const resolveTracked = (path, details) => {
    ...
    const child = toKey((0, _nodePath.join)(watched, path));
  ...
  watcher.on("raw", (event, path, details) => {
    ...
    const timeout = setTimeout(() => {
      pending.delete(file);
      if (disposed) return;
      (0, _promises.stat)(file).then((stats) => {
        if (disposed) return;
...
* @internal
*/
const buildDiagnostics = exports.buildDiagnostics = /* #__PURE__ */(0, _nostics.defineDiagnostics)({
  docsBase,
  reporters,
  codes: {
    NUXT_B1001: {
...
* @internal
*/
const configDiagnostics = exports.configDiagnostics = /* #__PURE__ */(0, _nostics.defineDiagnostics)({
  docsBase,
  reporters,
  codes: {
    NUXT_B5001: {
...
* @internal
*/
const headDiagnostics = exports.headDiagnostics = /* #__PURE__ */(0, _nostics.defineDiagnostics)({
  docsBase,
  reporters,
  codes: {
    NUXT_B6001: {
...
* @internal
*/
const bundlerDiagnostics = exports.bundlerDiagnostics = /* #__PURE__ */(0, _nostics.defineDiagnostics)({
  docsBase,
  reporters,
  codes: {
    NUXT_B7001: {
