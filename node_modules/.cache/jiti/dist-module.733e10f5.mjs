...
var _devtoolsKit = await jitiImport("@nuxt/devtools-kit");
var _mlly = await jitiImport("mlly");
var _nodePath = await jitiImport("node:path");
var _localPkg = await jitiImport("local-pkg");
var _iconVmJ4USKj = await jitiImport("./shared/icon.VmJ4USKj.mjs");
var _stdEnv = await jitiImport("std-env");
await jitiImport("consola");
await jitiImport("tinyglobby");
await jitiImport("@iconify/utils/lib/svg/parse");
await jitiImport("picomatch");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
...
function getResolvePaths(nuxt) {
  const layerDirs = (0, _kit.getLayerDirectories)(nuxt).map(
    (dir) => dir.root
  );
  return Array.from(
    new Set(
...
function registerServerBundle(ctx) {
  const { nuxt } = ctx;
  const templateServer = (0, _kit.addTemplate)({
    ...
    async getContents() {
      ...
      const collectionsValues = collections.map((collection) => {
        if (typeof collection === "string") {
          ...
          const path = (0, _iconVmJ4USKj.b)(collection, resolvePaths);
          if (!isBundling) {
            return `  '${collection}': () => require('${path}'),`;
          }
          const file = (0, _localPkg.resolveModule)(path, { paths: [nuxt.options.rootDir] }) ? void 0 : (0, _iconVmJ4USKj.f)(collection, resolvePaths);
          if (file) {
            const relPath = (0, _nodePath.relative)(nuxt.options.buildDir, file).replaceAll("\\", "/");
...
${failed.map((f) => " - " + f).join("\n")}`;
        ...
        _kit.logger.warn(msg);
      }
      if (dropped.length) {
        _kit.logger.debug(`Nuxt Icon could not resolve these icons for the client bundle, falling back to runtime loading:
${dropped.map((f) => " - " + f).join("\n")}`);
      }
      const { code, bundleSizeKb } = (0, _iconVmJ4USKj.g)(collections, { sizeLimitKb });
      if (collections.length)
      _kit.logger.info(`Nuxt Icon client bundle consist of \`${count}\` icons with \`${bundleSizeKb.toFixed(2)}KB\`(uncompressed) in size`);
      cacheData = code;
      cacheSize = count;
...
class NuxtIconModuleContext {
  ...
  getRuntimeCollections(runtimeOptions) {
    const resolved = runtimeOptions.fallbackToApi ? _iconVmJ4USKj.c : typeof this.options.serverBundle === "string" ? _iconVmJ4USKj.c : this.options.serverBundle ? this.options.serverBundle.collections?.map((c) => typeof c === "string" ? c : c.prefix) || [] : [];
  ...
  async _resolveServerBundle() {
    ...
    if (serverBundle === "auto") {
      const preset = this._nitroPreset || (typeof this.nuxt.options.nitro.preset === "string" ? this.nuxt.options.nitro.preset || _stdEnv.provider : _stdEnv.provider);
      serverBundle = "local";
      ...
      serverBundle = "remote";
      _kit.logger.info(`Nuxt Icon server bundle mode is set to \`${serverBundle}\``);
    }
    ...
    resolved.collections = resolved.remote ? _iconVmJ4USKj.c : await (0, _iconVmJ4USKj.d)(getResolvePaths(this.nuxt));
    const collections = await Promise.all(
      (resolved.collections || []).map((c) => (0, _iconVmJ4USKj.e)(c, this.nuxt.options.rootDir))
    );
    return {
      disabled: false,
  ...
  async _loadCustomCollection() {
    return Promise.all(
      (this.options.customCollections || []).map((collection) => (0, _iconVmJ4USKj.l)(collection, this.nuxt.options.rootDir))
    );
  }
  async loadClientBundleCollections() {
    ...
    if (scan && !this.scanner) {
      ...
      this.scanner = new _iconVmJ4USKj.I(scanOptions);
...
const module$1 = exports.default = (0, _kit.defineNuxtModule)({
  ...
  async setup(options, nuxt) {
    const resolver = (0, _kit.createResolver)("file:///C:/Users/Leonid/Desktop/Projects/food-menu/node_modules/@nuxt/icon/dist/module.mjs");
    const usesNitroV3 = Number.parseInt((0, _kit.getNuxtVersion)(nuxt)) >= 5;
    ...
    nuxt.hook("nitro:config", async (nitroConfig) => {
      ...
      return;
      _kit.logger.warn("Nuxt Icon's `serverBundle.externalizeIconsJson` is an experimental feature, it requires that your production Node.js server is able to import JSON modules.");
      const collections = bundle.collections.filter((collection) => typeof collection === "string").map((collection) => (0, _iconVmJ4USKj.b)(collection, getResolvePaths(nuxt)));
      const resolvedPaths = await Promise.all(
        collections.map((collection) => (0, _mlly.resolvePath)(collection, {
    ...
    if ((0, _kit.hasNuxtModule)("@unocss/nuxt"))
...
async function setupCustomCollectionsWatcher(options, nuxt, ctx) {
  ...
  return;
  let viteDevServer;
  const collectionDirs = await Promise.all(options.customCollections.filter((x) => "dir" in x).map((x) => (0, _kit.resolvePath)(x.dir)));
  ...
  nuxt.hook("builder:watch", async (event, path) => {
    const resolvedPath = await (0, _kit.resolvePath)(path);
    if (ctx.scanner) {
      ctx.scanner.extractFromCode(
        await _promises.default.readFile(resolvedPath, "utf-8").catch(() => ""),
        ctx.scannedIcons
      );
    }
    if (collectionDirs.some((cd) => resolvedPath.startsWith(cd))) {
      await ctx.loadCustomCollection(true);
      await (0, _kit.updateTemplates)({
